...
ERROR_CORRECTION = "M"   # Use high error correction to tolerate center logo
BORDER = 4               # Quiet zone modules (2–4 typical)
SCALE = 12               # Pixel scale per module (increase for higher resolution)

# Logo configuration
LOGO_PATH = None  # Disable logo overlay (no logo embedded in SVG or JPG)
LOGO_SCALE = 0.3
LOGO_PAD = 1
# -------------------------------
...
def main():
    ...
    csv_path = Path(sys.argv[1]) if len(sys.argv) > 1 else Path(__file__).with_name("qrcoderequest.csv")
    ...
    with csv_path.open("r", encoding="utf-8-sig", newline="") as f:
        ...
        for row in reader:
            ...
            # Use the provided URL as-is (do not add UTM parameters)
            tracked_url = url
